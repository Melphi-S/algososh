...
export const StringComponent: React.FC = () => {
    ...
    const visualizeAlgorithm = async () => {
        ...
        const elements = value.split('').map((char, index) => ({char, id: index, state: ElementStates.Default}));
        ...
        <SolutionLayout title="Строка">
            <div className={styles.wrapper}>
                <Input value={value}
                       ...
                       maxLength={11} isLimitText
                       extraClass={styles.input
                       }/>
                <Button onClick={visualizeAlgorithm}
                        text='Развернуть'
                        extraClass={styles.button}
                        isLoader={isButtonBlocked}>Развернуть</Button>
                <div className={styles.algorithm}>
                    {sortingElements.length > 0 && sortingElements.map(element => <Circle letter={element.char}
                                                                                          key={element.id}
                                                                                          state={element.state}/>)}
                </div>
            </div>
